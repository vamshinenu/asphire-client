    ...
    return (
        <div className='text-center'>
            ...
            <button className='px-8 max-w-96' onClick={() => signIn('google', { callbackUrl })}>
                <div className='flex flex-row items-center justify-center gap-4 px-8 py-4 text-white duration-500 bg-transparent rounded-lg hover:rounded-3xl outline outline-1 group hover:text-black hover:bg-white'>
                    <h1 className="text-center">Sign in with</h1>
                    <FontAwesomeIcon icon={faGoogle} className="w-6 h-6" />
                </div>
            </button>
            <p className="max-w-md px-4 mt-2 text-xs text-red-500 ">{searchParams.get('error') !== null ? 'We are facing trouble logging you in, Try checking your internet connection or clear cookies and try again' : ''}</p>
